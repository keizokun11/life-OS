...
function validTimeText(x){ return /^([01]\d|2[0-3]):[0-5]\d$/.test(String(x||'')); }
function dateTimeFromDayAndAbsoluteMinute(day, absMinute){
  const offset = Math.floor(absMinute / 1440);
  const minute = ((Math.round(absMinute) % 1440) + 1440) % 1440;
  return `${addDays(day, offset)}T${timeLabel(minute)}:00`;
}
function addQuickEventFromForm(day, data){
  const title=String(data.quickTitle||'急な用事').trim()||'急な用事';
  const startTime=String(data.quickStart||'');
  const inputMode=String(data.quickInputMode||'range');
  const bufferLevel=data.quickBuffer||'small';
  if(!validTimeText(startTime)) return notice('開始時刻を入力してください。');
  const startMin=dayMinuteFromClock(startTime);
  let duration=0, endAbsMin=0, endTimeLabel='';
  if(inputMode==='duration'){
    duration=Math.round(Number(data.quickDuration||0));
    if(!Number.isFinite(duration)||duration<=0) return notice('必要時間を1分以上で入力してください。');
    if(duration>24*60) return notice('必要時間は24時間以内にしてください。');
    endAbsMin=startMin+duration;
    endTimeLabel=timeLabel(endAbsMin);
  } else {
    const endTime=String(data.quickEnd||'');
    if(!validTimeText(endTime)) return notice('終了時刻を入力してください。');
    const endMin=dayMinuteFromClock(endTime);
    endAbsMin=endMin<=startMin ? endMin+1440 : endMin;
    duration=endAbsMin-startMin;
    endTimeLabel=endTime;
  }
  if(duration<1) return notice('用事の時間を入力してください。');
  const item={id:`quick:${uid()}`,title,allDay:false,start:dateTimeFromDayAndAbsoluteMinute(day,startMin),end:dateTimeFromDayAndAbsoluteMinute(day,endAbsMin),bufferLevel,source:'quick',inputMode,durationMinutes:duration,createdAt:new Date().toISOString()};
  quickEvents=[...(quickEvents||[]),item];
  recordOperation('quick_event_created','急な用事を追加',{eventId:item.id,title,startTime,endTime:endTimeLabel,durationMinutes:duration,bufferLevel,inputMode},day);
  persist(); renderAll(); notice('急な用事を入れて、今日の予定を再計算しました。');
}
...
function markAwakeNow(){
  ...
  persist(); renderAll(); notice(`起床 ${wakeTime}。この時刻から朝トレ・食事・予定・課題を再計算しました。`);
}
function editActualWakeTime(day=dateKey()){
  ...
  const base=new Date(dateTimeFromDayAndAbsoluteMinute(day, dayMinuteFromClock(answer))), sleep=sleepPlanForDay(day), before=wakeRecords[day]?.wakeTime||'';
...
function wakeGateMarkup(tab='today'){
  ...
  return `<div class="stack"><section class="hero-card wake-gate"><p class="eyebrow">MORNING START</p><h2>起きたら、ここから1日を開始</h2><div class="wake-times"><div><span>起床予定</span><strong>${sleep.plannedWakeTime}</strong></div><div><span>現在時刻</span><strong>${now}</strong></div></div><button id="wakeNow-${tab}" class="primary wake-button">起きた（${now}）</button><p class="wake-note">このボタンを押すまでは今日の作業計画を作成しません。押した実際の起床時刻から、朝トレ・朝食・昼食・夕食も含めて今日の予定を組み直します。朝の予定に間に合わない場合は自動で短縮して、その後に授業・予定・課題・ゆったり時間を再計算します。</p></section></div>`;
}
...
function currentPlan(day=selectedDay){
  ...
  if(day === dateKey() && wakeRecords[day]?.wakeTime){
    const now = new Date();
    st.rescheduleMovableFromMinute = dayMinuteFromDate(now);
  }
...
function nowTimelineMarkup(plan, day=dateKey(), nowMin=null){
  ...
  const currentMin = nowMin ?? (()=>dayMinuteFromDate(new Date()))();
...
function parseSmartInputLocal(text, baseDay=selectedDay||dateKey()){
  ...
  const mealMatch=raw.match(/(朝食|昼食|夕食|朝ごはん|昼ごはん|夜ごはん|晩ごはん).*?([0-2]?\d:[0-5]\d).*?(\d+)分?/);
  if(mealMatch){ const mealName=mealMatch[1].includes('朝')?'breakfast':mealMatch[1].includes('昼')?'lunch':'dinner'; actions.push({type:'mealSetting',meal:mealName,time:mealMatch[2],minutes:Number(mealMatch[3]),summary:`${mealName==='breakfast'?'朝食':mealName==='lunch'?'昼食':'夕食'}を${mealMatch[2]}から${mealMatch[3]}分に設定`}); }
  if(!actions.length) actions.push({type:'unknown',summary:'うまく解釈できませんでした。例：23:30から洗濯30分 / 明日までにIELTS20ページ / 今日は朝トレなし / 昼食12:30から30分'});
  return actions;
}
...
function applySmartAction(action){
  ...
  if(action.type==='mealSetting'){
    const before=JSON.parse(JSON.stringify(settings.meals||{}));
    settings.meals={...(settings.meals||{})};
    if(action.meal==='breakfast'){
      settings.morningTraining={...(settings.morningTraining||{}),breakfastEnabled:true,breakfastMinutes:action.minutes};
      settings.meals.breakfast={...(settings.meals.breakfast||{}),enabled:true,minutes:action.minutes};
    } else {
      settings.meals[action.meal]={...(settings.meals[action.meal]||{}),enabled:true,time:action.time,minutes:action.minutes};
    }
    recordOperation('ai_input_applied','AI入力から食事時間を反映',{before,after:settings.meals,meal:action.meal},selectedDay);
    persist(); renderAll(); notice('AI入力から食事時間を反映して再計算しました。'); return;
  }
  if(action.type==='task'){
...
function renderToday(){
  ...
  if($('quickEventForm')){
    const form=$('quickEventForm'), mode=$('quickInputMode');
    const syncQuickMode=()=>{ const durationMode=mode?.value==='duration'; form.querySelectorAll('.quick-range-field').forEach(x=>x.classList.toggle('hidden',durationMode)); form.querySelectorAll('.quick-duration-field').forEach(x=>x.classList.toggle('hidden',!durationMode)); };
    if(mode){ mode.onchange=syncQuickMode; syncQuickMode(); }
...
function renderSettings(){
  ...
  if($('breakfastEnabled')) $('breakfastEnabled').onchange=e=>{const before=settings.morningTraining?.breakfastEnabled!==false;settings.morningTraining={...(settings.morningTraining||{}),breakfastEnabled:e.target.checked};settings.meals={...(settings.meals||{}),breakfast:{...(settings.meals?.breakfast||{}),enabled:e.target.checked}};recordOperation('breakfast_toggled','朝食設定を変更',{before,after:e.target.checked});persist();renderToday();renderNow();renderSettings();};
  if($('breakfastEnabledMeal')) $('breakfastEnabledMeal').onchange=e=>{const before=settings.morningTraining?.breakfastEnabled!==false;settings.morningTraining={...(settings.morningTraining||{}),breakfastEnabled:e.target.checked};settings.meals={...(settings.meals||{}),breakfast:{...(settings.meals?.breakfast||{}),enabled:e.target.checked}};recordOperation('breakfast_toggled','朝食設定を変更',{before,after:e.target.checked});persist();renderToday();renderNow();renderSettings();};
  if($('lunchEnabled')) $('lunchEnabled').onchange=e=>{const before=settings.meals?.lunch?.enabled!==false;settings.meals={...(settings.meals||{}),lunch:{...(settings.meals?.lunch||{}),enabled:e.target.checked}};recordOperation('meal_setting_changed','昼食設定を変更',{meal:'lunch',before,after:e.target.checked});persist();renderToday();renderNow();renderSettings();};
  if($('dinnerEnabled')) $('dinnerEnabled').onchange=e=>{const before=settings.meals?.dinner?.enabled!==false;settings.meals={...(settings.meals||{}),dinner:{...(settings.meals?.dinner||{}),enabled:e.target.checked}};recordOperation('meal_setting_changed','夕食設定を変更',{meal:'dinner',before,after:e.target.checked});persist();renderToday();renderNow();renderSettings();};
